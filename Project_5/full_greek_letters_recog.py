...
def read_model(file_path):
    ...
    try:
        model = MyNetwork(dropout_rate=0.3, flatten_size=3)
...
def train_greek_network(file_path, image_dir):
    ...
    ### Replace the last layer with a new Linear layer with 26 nodes
    model.fc2 = nn.Linear(50, 26)  # Output features -> 26 nodes
    ...
    torch.save(model.state_dict(), './Project_5/results/full_greek_model.pth')
...
def evaluate_greek_model_from_folder(model_path, test_folder_path):
    ...
    model = MyNetwork()
    model.fc2 = nn.Linear(50, 26)  # Adjust the last layer
   ...
   # Evaluate and display only the first 12 test images
    fig = plt.figure(figsize=(10, 10))
    rows, cols = 3, 4  # Display images in a 3x4 grid

    for idx, (data, target) in enumerate(test_loader):
        if idx >= 12:  # Stop after processing the first 12 images
            break
...
if __name__ == "__main__":
    ...
    evaluate_greek_model_from_folder('./Project_5/results/full_greek_model.pth', args.test_dir)
